...
export const dashboard = async (req, res, next) => {
  try {
    ...
    const añoEscolar = await db.ConfiguracionSistema.getValor('año_escolar_actual');
    ...
    const asistenciasEstudiantes = await db.AsistenciaEstudiante.findAll({
      where: { fecha },
      attributes: ['estado', [db.sequelize.fn('COUNT', db.sequelize.col('id')), 'count']],
      group: ['estado'],
      raw: true
    });
    ...
    const resumenPersonal = {
      docentes_aula: { total: 0, presentes: 0, ausentes: 0, tardanzas: 0, justificados: 0 },
      directivo: { total: 0, presentes: 0, ausentes: 0, tardanzas: 0, justificados: 0 },
      administrativo: { total: 0, presentes: 0, ausentes: 0, tardanzas: 0, justificados: 0 }
    };

    // Contar totales por categoría
    personalActivo.forEach(usuario => {
      if (usuario.categoria_personal && resumenPersonal[usuario.categoria_personal]) {
    ...
    const progresoGrados = {
      ...
      porcentaje: totalGrados > 0 ? Math.round((gradosCompletados / totalGrados) * 100) : 0
    };
    ...
    const gradosPendientesDetalle = gradosPendientes
      .filter(grado => {
        ...
        const registro = grado.registrosAsistencia && grado.registrosAsistencia.length > 0;
        return !registro;
      })
      .map(grado => ({
        id: grado.id,
    ...
    // ============================================
    const puedeEnviarMinerd = gradosCompletados === totalGrados && totalGrados > 0;
...
export const validarAsistencia = async (req, res, next) => {
  try {
    ...
    logger.info(`Usuario ${req.usuario.email} validó asistencia del día ${fecha}`);
...
export const enviarRecordatorio = async (req, res, next) => {
  try {
    ...
    if (!docente_ids || !Array.isArray(docente_ids) || docente_ids.length === 0) {
    ...
    // Para cada docente, obtener sus grados pendientes
    for (const docente of docentes) {
      ...
      for (const asignacion of asignaciones) {
        ...
        if (!registro) {
          gradosPendientes.push(asignacion.grado);
        }
      }
    ...
    logger.info(`Usuario ${req.usuario.email} envió recordatorios manuales a ${docentes.length} docentes`);
...
export const modificarAsistencia = async (req, res, next) => {
  try {
    ...
    if (!estudiante_id || !fecha || !estado) {
      return errorResponse(res, 'estudiante_id, fecha y estado son requeridos', 400);
    }
    ...
    logger.info(`Usuario ${req.usuario.email} modificó asistencia - Estudiante ID: ${estudiante_id}`);

    return successResponse(res, asistencia, 'Asistencia modificada exitosamente');
  } catch (error) {
    next(error);
  }
};
...
export const listaAsistenciaPersonal = async (req, res, next) => {
  try {
    ...
    const personal = await db.Usuario.findAll({
      where: { activo: true },
      attributes: ['id', 'nombre', 'apellido', 'email', 'rol', 'categoria_personal'],
      order: [['apellido', 'ASC'], ['nombre', 'ASC']]
    });
